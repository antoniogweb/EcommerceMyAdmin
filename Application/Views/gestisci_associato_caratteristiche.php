...
<!-- 	<p><a class="btn btn-primary iframe pull-right" href="<?php echo $this->baseUrl."/caratteristiche/main?partial=Y"?>"><i class="fa fa-edit"></i> Gestione caratteristiche</a> -->
	...
	<div>
		...
		<form class="form-inline" role="form" action='<?php echo $this->baseUrl."/".$this->applicationUrl.$this->controller."/aggiungicaratteristica/$id_page".$this->viewStatus;?>' method='POST'>
			<?php echo Html_Form::input("titolo_car","","form-control auto","titolo_car",'data-provide="typeahead" autocomplete="off" placeholder="'.gtextAttr("Caratteristica").'" source="caratteristiche/elenco/'.$sezione.'"');?>
			<?php echo Html_Form::input("titolo_carval","","form-control auto","titolo_carval",'data-provide="typeahead" autocomplete="off" placeholder="'.gtextAttr("Valore").'" source="caratteristichevalori/elenco/'.$sezione.'"');?>
			<input class="submit_file btn btn-primary" type="submit" name="insertAction" value="<?php echo gtextAttr("Aggiungi");?>">
		</form>
		<?php } ?>
	</div>
	<br />
