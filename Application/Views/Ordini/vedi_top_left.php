...
<table class="table table-striped">
	...
	<tr>
		<td><?php echo gtextPlain("Agente");?>:</td>
		<td>
	...
	?>
		<tr>
			<td><?php echo gtextPlain("Pixel");?>:</td>
			<td>
				...
				?>
					...
					<?php echo gtextPlain("inviato in data/ora").": ".date("d-m-Y H:i", strtotime($ev["pixel_eventi"]["data_creazione"]));?> <i class="text text-success fa fa-thumbs-up"></i>
					<a class="iframe" title="<?php echo gtextAttr("Vedi codice script");?>" href="<?php echo $this->baseUrl."/ordini/vediscriptpixel/".$ev["pixel_eventi"]["id_pixel_evento"];?>?partial=Y&nobuttons=Y"><i class="fa fa-eye"></i></a>
					<br />
				<?php } ?>
			</td>
		</tr>
	...
	<tr>
		<td><?php echo gtextPlain("Spedizione");?>:</td>
		<td>
			<?php
			...
			<div>
				...
				<a class="iframe badge" href="<?php echo $this->baseUrl."/spedizioninegozio/form/insert/0?id_o=".$ordine["id_o"].$queryStringCreaSpedizioneLista;?>&partial=Y"><i class="fa fa-plus-square-o"></i> <?php echo gtextPlain("Crea spedizione");?></a>
			</div>
			<?php } ?>
		</td>
	</tr>
	...
	if (v("attiva_richiesta_reso_online")) { 
		...
		if (count($tabellaPeriodiReso) > 0 || $permettiReso) { ?>
			<tr>
				<td><?php echo gtextPlain("Periodi reso");?>:</td>
				<td>
					<?php foreach ($tabellaPeriodiReso as $i => $pr) { ?>
						<div class="box-reso">
							...
							<?php echo gtextPlain("Spedizione")." <b>".$pr["id_spedizione_negozio"]; ?></b> - 
							<?php } ?>
							...
							<?php if ($pr["richiesta"]) { ?>
							<br /><span class="text text-danger text-bold"><i class="fa fa-exclamation-circle"></i> <?php echo gtextPlain("Richiesta di reso effettuata in data")?> <?php echo smartDate($pr["data_richiesta"], v("default_date_format")." H:i");?><br /><?php echo gtextPlain("Identificativo richiesta");?>: <?php echo $pr["id_o_periodo_reso"];?></span>
							<?php } ?>
						</div>
					<?php } ?>
					<?php if ($permettiReso) { ?>
					<a class="iframe label label-info" href="<?php echo $this->baseUrl."/ordiniperiodireso/form/insert/0";?>?partial=Y&id_o=<?php echo $ordine["id_o"];?>&nobuttons=Y"><i class="fa fa-plus-circle"></i> <?php echo gtextPlain("Aggiungi periodo")?></a>
					<?php } ?>
				</td>
			</tr>
		<?php } ?>
