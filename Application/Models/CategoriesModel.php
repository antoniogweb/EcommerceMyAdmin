...
// 	}
	...
	public function indentList($id, $alias = true, $editLink = true, $useHtml = true)
	{
		...
		if ($this->section && !isset($_GET["esporta"]))
			$str = "<i style='position:relative;left:-20px;top:1px;' class='toogle_category_tree fa ".UsersopzioniModel::gOpz($id, v("fawe_folder_default_class"), array(v("fawe_folder_closed_class"), v("fawe_folder_opened_class")))."' aria-hidden='true'></i> <div class='record_id' style='display:none'>$id</div><i title='".gtextAttr("Trascina per ordinare")."' class='ancora_ordinamento fa fa-arrows text text-warning' style='padding-right:3px;font-size:12px;'></i>";
