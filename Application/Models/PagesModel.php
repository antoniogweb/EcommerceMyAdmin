...
// 		$this->hModel = new $this->hModelName();
		...
		if (v("attiva_liste_regalo"))
		{
			...
			self::$tipiPagina["RICHIEDI_RESO_LISTA"] = "Pagina per richiedere un reso di una lista";
		}
...
// 		print_r(self::$preloadedPages);
		...
		if (!VariabiliModel::attivaLinguaRicaduta($lingua) && isset(self::$preloadedPages[$clean["id"]]))
		{
...
// 			print_r($parents);
	...
	public function addJoinTraduzionePagina($lingua = null, $usaDistinct = true, $soloTraduzioni = false)
	{
		...
		$this->left("contenuti_tradotti")->left("contenuti_tradotti as contenuti_tradotti_categoria");
		
		if ($linguaNonSettata && VariabiliModel::attivaLinguaRicaduta($lingua))
			$this->on(array("contenuti_tradotti.id_page = pages.id_page and (
				(contenuti_tradotti.lingua = ? and contenuti_tradotti.salvato = 1) OR 
				contenuti_tradotti.lingua = ? AND NOT EXISTS (
					SELECT 1
					FROM contenuti_tradotti AS ct
					WHERE ct.id_page = pages.id_page
					AND ct.lingua = ?
					AND ct.salvato = 1
				)
			)", array(
				sanitizeDb($lingua),
				sanitizeDb(v("usa_lingua_se_manca_traduzione")),
				sanitizeDb($lingua),
			)))->on(array("contenuti_tradotti_categoria.id_c = categories.id_c and (
				(contenuti_tradotti_categoria.lingua = ? and contenuti_tradotti_categoria.salvato = 1) OR 
				contenuti_tradotti_categoria.lingua = ? AND NOT EXISTS (
                SELECT 1
					FROM contenuti_tradotti AS ctc
					WHERE ctc.id_c = categories.id_c
					AND ctc.lingua = ?
					AND ctc.salvato = 1
				)
			)", array(sanitizeDb($lingua), sanitizeDb(v("usa_lingua_se_manca_traduzione")), sanitizeDb($lingua))));
		else
			$this->on(array("contenuti_tradotti.id_page = pages.id_page and contenuti_tradotti.lingua = ?", array(sanitizeDb($lingua))))->on(array("contenuti_tradotti_categoria.id_c = categories.id_c and contenuti_tradotti_categoria.lingua = ?", array(sanitizeDb($lingua))));
