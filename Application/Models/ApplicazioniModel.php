	...
	public function daaggiornare($record)
	{
		if (file_exists(ROOT."/Application/Apps/".ucfirst($record["applicazioni"]["codice"])."/DB/Migrazioni"))
		{
			...
			if ($migrationNum > (int)$record["applicazioni"]["db_version"])
				return "<a class='iframe' title='".gtextAttr("Aggiorna il database")."' href='".Url::getRoot()."applicazioni/migrazioni/".v("codice_cron")."/".$record["applicazioni"]["id_applicazione"]."?partial=Y'><i class='fa fa-refresh text text-warning'></i></a>";
		}
