...
// 			));
	...
	public function buildAllPagesSelect()
	{
		...
		return array("0"=>gtext("-- NON IMPOSTATO --")) + $p->clear()->select("id_page,title")->orderBy("title")->toList("id_page","title")->send();
	}
		...
		if (count($fasce) > 0)
		{
			foreach ($fasce as $f)
			{
				...
				if (User::$adminLogged)
				{
					...
					$htmlFinale .= " <a title='".gtextAttr("Modifica fascia")."' style='margin-left:5px;' class='class_edit_fascia iframe' href='".Url::getFileRoot()."admin/contenuti/form/update/".$idCont."?partial=Y'><i class='fa fa-pencil'></i></a>";
					
					if (TestiModel::numero($idCont))
						$htmlFinale .= " <a title='".gtextAttr("Edita testi fascia")."' style='margin-left:5px;' class='class_edit_fascia iframe' href='".Url::getFileRoot()."admin/testi/main?partial=Y&id_cont=".self::$idContenuto."&lingua=".Params::$lang."'><i class='fa fa-font'></i></a>";
					
// 					$htmlFinale .= " <a title='".gtextAttr("Ordina / Aggiungi fasce")."' style='margin-left:5px;' class='iframe' href='".Url::getFileRoot()."admin/$controller/contenuti/".(int)$idElemento."?partial=Y&nobuttons=Y'><i class='fa fa-sort'></i></a>";
