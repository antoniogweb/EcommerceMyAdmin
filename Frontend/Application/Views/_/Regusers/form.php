...
<form class="form_registrazione box_form_evidenzia" action="<?php echo $this->baseUrl.$action;?>#main" method="POST" autocomplete="new-password">
	...
	<div class="<?php echo $classeBoxPulsanteRegistrazione;?>">
		...
		<?php if (strcmp($this->action,"modify") !== 0 || (v("attiva_gestiobe_ticket") && RegusersModel::getRedirect() == "ticket")) { ?>
			<input class="<?php echo v("classe_pulsanti_submit");?> btn_submit_form uk-width-1-1 uk-width-auto@m" type="submit" name="updateAction" value="<?php echo gtextAttr("Completa registrazione", false);?>" />
		<?php } else { ?>
			<?php if (v("permetti_modifica_account")) { ?>
				<input class="<?php echo v("classe_pulsanti_submit");?> btn_submit_form uk-width-1-1 uk-width-auto@m" type="submit" name="updateAction" value="<?php echo gtextAttr("Modifica dati", false);?>" />
			<?php } ?>
		<?php } ?>
	</div>
</form>
