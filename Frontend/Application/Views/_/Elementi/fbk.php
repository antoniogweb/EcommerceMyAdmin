		...
		setTimeout(function(){
			...
			if (isset($idOrdineGtm))
			{
				...
				if (!empty($ordineGTML) && (!$ordineGTML["inviato_fbk"] || $ordineGTML["pagamento"] == "paypal") && $ordineGTML["stato"] != "deleted")
				{
					...
					foreach ($rOrdine as $ro)
					{
						...
						$tempRigheGTM[] = array(
							"id"	=>	v("usa_sku_come_id_item") ? FeedModel::cXF("FACEBOOK",$ro["codice"]) : $ro["id_page"],
			...
			else
			{
				...
				if (isset($isPage) && $idPaginaPerTracking && isProdotto($idPaginaPerTracking)) {
					$arrayProprieta["content_ids"] = v("usa_sku_come_id_item") ? array(FeedModel::cXF("FACEBOOK",$codicePerTracking)) : array($idPaginaPerTracking);
				...
				<?php } else if ($this->controller == "ordini" && $this->action == "index") {
					...
					{
						$tempRigheGTM[] = array(
							"id"	=>	v("usa_sku_come_id_item") ? FeedModel::cXF("FACEBOOK",$p["cart"]["codice"]) : $p["cart"]["id_page"],
