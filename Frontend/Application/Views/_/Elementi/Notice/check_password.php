...
<ul>
	...
	<li><?php echo gtext("Deve contenere almeno un carattere maiuscolo [A - Z]");?></li>
	<?php } ?>
	<?php if (!preg_match('/^'.v("password_regular_expression_caratteri_numerici").'.{1,}$/',$_POST["password"])) {  ?>
	<li><?php echo gtext("Deve contenere almeno un carattere numerico [0 - 9]");?></li>
	<?php } ?>
	<?php if (!preg_match('/^(?=.*?['.v("password_regular_expression_caratteri_speciali").']).{1,}$/',$_POST["password"])) {  ?>
	<li><?php echo gtext("Deve contenere almeno un carattere speciale tra i seguenti:")." ".v("password_regular_expression_caratteri_speciali");?></li>
	<?php } ?>
</ul>
<?php } ?>
