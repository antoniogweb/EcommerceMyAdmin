...
(function($) {
	...
	function ProductImagePicker(select, options) {
		this.$select = $(select);
		this.options = $.extend({ hide_select: true, selected: null, changed: null, clicked: null, initialized: null }, options);
		this.multiple = this.$select.prop("multiple");
		this.build();
	}

	ProductImagePicker.prototype.build = function() {
		var picker = this;
		this.$picker = $("<ul>", { "class": "thumbnails image_picker_selector", role: "listbox", "aria-multiselectable": this.multiple ? "true" : "false" });
		this.$select.children("option").each(function() {
			var $option = $(this);
			var source = $option.data("img-src");
			if (!source) return;
			var label = $option.data("img-label") || $option.data("img-title") || $option.text();
			var $button = $("<button>", { type: "button", "class": "thumbnail", role: "option", "aria-label": label, "aria-pressed": "false" });
			$button.append($("<img>", { "class": "image_picker_image", src: source, alt: $option.data("img-alt") || label }));
			$button.on("click", function(event) { picker.select($option, event); });
			picker.$picker.append($("<li>").append($button).data("productImageOption", $option));
		});
		this.$select.after(this.$picker).on("change.productImagePicker", function() { picker.sync(); });
		if (this.options.hide_select) this.$select.hide();
		this.sync();
	};

	ProductImagePicker.prototype.values = function() {
		var value = this.$select.val();
		return this.multiple ? (value || []) : [value];
	};

	ProductImagePicker.prototype.sync = function() {
		var values = $.map(this.values(), String);
		this.$picker.children("li").each(function() {
			var $item = $(this);
			var selected = $.inArray(String($item.data("productImageOption").val()), values) !== -1;
			$item.find(".thumbnail").toggleClass("selected", selected).attr("aria-pressed", selected ? "true" : "false");
		});
	};

	ProductImagePicker.prototype.select = function($option, event) {
		var oldValues = this.values();
		var value = String($option.val());
		if (this.multiple) {
			var values = $.map(oldValues, String), index = $.inArray(value, values);
			if (index === -1) values.push(value); else values.splice(index, 1);
			this.$select.val(values);
		} else this.$select.val(value);
		this.$select.trigger("change");
		if ($.isFunction(this.options.clicked)) this.options.clicked.call(this.$select, $option, event);
		if ($.isFunction(this.options.selected) && $option.is(":selected")) this.options.selected.call(this.$select, $option, event);
		if ($.isFunction(this.options.changed)) this.options.changed.call(this.$select, oldValues, this.values(), event);
	};

	ProductImagePicker.prototype.destroy = function() { this.$select.off("change.productImagePicker").show().removeData("picker"); this.$picker.remove(); };
	window.initProductImagePicker = function($selects, options) {
		return $selects.each(function() {
			var $select = $(this), previous = $select.data("picker");
			if (previous) previous.destroy();
			var picker = new ProductImagePicker(this, options || {});
			$select.data("picker", picker);
			if ($.isFunction(picker.options.initialized)) picker.options.initialized.call(picker);
		});
	};
})(jQuery);
