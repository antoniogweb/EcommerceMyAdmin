...
return;if(ricarica_pagamenti===undefined&&$(".bx_pagamenti").length>0){var pagamento=$("[name='pagamento']:checked").val();$.ajaxQueue({url:baseUrl+"/ordini/pagamenti/"+nazione,cache:!1,async:!0,dataType:"json",success:function(content){if(!isInArray(pagamento,content))
pagamento=content[0];if(content.length==0){$(".radio_pagamento").each(function(){$(this).find("input").iCheck('uncheck')});$(".bx_pagamenti").css("display","none")}else $(".bx_pagamenti").css("display","block");$(".radio_pagamento").css("display","none");for(var i=0;i<content.length;i++){$(".radio_pagamento.radio_pagamento_"+content[i]).css("display","block");if(pagamento==content[i]){cercaSpeseSpedizione=!1;$(".radio_pagamento.radio_pagamento_"+content[i]).find("input").iCheck('check');cercaSpeseSpedizione=!0}}
