...
return;if(ricarica_pagamenti===undefined&&$(".bx_pagamenti").length>0){var pagamento=$("[name='pagamento']:checked").val();$.ajaxQueue({url:baseUrl+"/ordini/pagamenti/"+nazione,cache:!1,async:!0,dataType:"json",success:function(content){$(".radio_pagamento").each(function(){$(this).find("input").iCheck('uncheck')});if(content.length==0)
