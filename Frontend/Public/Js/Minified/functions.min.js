...
var attiva_icheck=!0;$=jQuery;function isInArray(elemento,haystack){var count=haystack.length;for(var i=0;i<count;i++){if(haystack[i]==elemento){return!0}}
return!1}
...
if(typeof svuotaCampiSpedizione!=='function'){window.svuotaCampiSpedizione=function(){$("[name='indirizzo_spedizione']").val("");$("[name='cap_spedizione']").val("");$("[name='citta_spedizione']").val("");$("[name='provincia_spedizione']").val("");$("[name='dprovincia_spedizione']").val("");$("[name='telefono_spedizione']").val("");$("[name='nazione_spedizione']").val("IT");if($("[name='destinatario_spedizione']").length>0)
...
function checkCouponAttivo(){$.ajaxQueue({url:baseUrl+"/ordini/couponattivo",cache:!1,async:!0,dataType:"html",success:function(content){if($.trim(content)=="OK")
$(".box_coupon").remove();}})}
...
var cercaSpeseSpedizione=!0;function impostaCorrieriESpeseSpedizione(){if(!cercaSpeseSpedizione)
return;var nazione=getNazione();if(nazione==""||nazione==undefined)
return;if(attiva_spedizione){if($(".box_corrieri").length>0){var id_corriere=$("[name='id_corriere']:checked").val();$.ajaxQueue({url:baseUrl+"/ordini/corrieri/"+nazione,cache:!1,async:!0,dataType:"json",success:function(content){if(!isInArray(id_corriere,content))
...
if(typeof evidenziaErrore!=='function'){window.evidenziaErrore=function(selettore,form){var nomeCampo=selettore.substring(7);if($(".box_form_evidenzia").length>0){var boxErrori=(typeof form=="undefined")?$(".evidenzia").closest(".box_form_evidenzia"):form.closest(".box_form_evidenzia");if(boxErrori.find(selettore).length>0)
...
$("body").on("click",".btn_completa_acquisto",function(e){mostraSpinner($(this));if(gtm_analytics&&typeof checkout_items!=="undefined"){if(debug_js)
console.log(checkout_items);gtag('event','checkout_progress',checkout_items)}});$("body").on("click",".btn_submit_form",function(e){mostraSpinner($(this))});if(attiva_icheck)
$('input').iCheck(icheckOptions);mostraLabelColore();if($(".image-picker").length>0){$(".image-picker").imagepicker({selected:function(select,picker_option,event){mostraLabelColore()}})}
...
impostaCorrieriESpeseSpedizione();else aggiornaCarrello(undefined,!0)}})});$("body").on("click","[name='invia_coupon']",function(e){if(coupon_ajax&&$(".cart_container").length==0){e.preventDefault();var url=$(this).closest("form").attr("action");var il_coupon=$(this).closest("form").find("[name='il_coupon']").val();var that=$(this);that.addClass("uk-hidden").parent().find(".spinner").removeClass("uk-hidden");$.ajaxQueue({url:url,cache:!1,async:!0,dataType:"html",method:"POST",data:{invia_coupon:"invia_coupon",il_coupon:il_coupon},success:function(content){setTimeout(function(){that.removeClass("uk-hidden").parent().find(".spinner").addClass("uk-hidden")},500);impostaCorrieriESpeseSpedizione();checkCouponAttivo()}})}});$("body").on("submit","form",function(e){var form=$(this);if(form.find("[name='ajaxsubmit']").length>0){var boxEvidenzia=form.closest(".box_form_evidenzia");if(boxEvidenzia.length>0&&boxEvidenzia.find(".box_notice").length>0){e.preventDefault();var url=form.attr("action");$.ajaxQueue({url:url,cache:!1,async:!0,dataType:"json",method:"POST",data:form.serialize(),success:function(content){if($(".btn_submit_form").length>0){setTimeout(function(){$(".btn_submit_form").removeClass("uk-hidden").parent().find(".spinner").addClass("uk-hidden")},500)}
boxEvidenzia.find(".box_notice").html(content.Body.Notice);if(content.Body.Esito=="OK"){form.remove()}
...
if(y>(offset3.top-200)){mostraPosizioneStepCheckout("pagamento")}else{nascondiPosizioneStepCheckout("pagamento")}
...
if(y>(offset5.top-200)){console.log("aaa");mostraPosizioneStepCheckout("conferma")}else{nascondiPosizioneStepCheckout("conferma")}})}})