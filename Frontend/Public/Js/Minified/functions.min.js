...
$('input:not(.no_icheck_input)').iCheck(icheckOptions);mostraLabelColore();if($(".image-picker").length>0){initProductImagePicker($(".image-picker"),{selected:function(select,picker_option,event){mostraLabelColore()}})}
...
location.href=baseUrl+"/checkout";else aggiornaCarrello(undefined,!0)}})});$(".showcoupon").click(function(e){e.preventDefault();$("#coupon").slideToggle()});$(".showlogin").click(function(e){e.preventDefault();$("#login").slideToggle()});if(filtro_prezzo_slider){$('.nstSlider').each(function(){var slider=$(this);var aggiornaEtichette=function(values){slider.find('.leftLabel').text(values[0]+"€");slider.find('.rightLabel').text(values[1]+"€")};slider.slider({range:!0,min:Number(slider.data('range_min')),max:Number(slider.data('range_max')),values:[Number(slider.data('cur_min')),Number(slider.data('cur_max'))],create:function(event,ui){slider.find('.ui-slider-range').addClass('bar');aggiornaEtichette(slider.slider('values'))},slide:function(event,ui){aggiornaEtichette(ui.values)},stop:function(event,ui){var urlSlider=$(".url_slider_prezzo").text();urlSlider=urlSlider.replace("[DA]",ui.values[0]);urlSlider=urlSlider.replace("[A]",ui.values[1]);location.href=urlSlider}})})}
