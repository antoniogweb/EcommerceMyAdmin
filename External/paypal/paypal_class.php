...
$logFilePath = FRONT."/Logs/.ipn_results.log";
...
class paypal_class {
	...
	public function submit_paypal_post() {
		...
		echo "<input style='display:none;' type=\"submit\" value=\"Click Here\"></center>\n";
		echo "<script>document.paypal_form.submit();</script>";
		echo "</form>\n";
		echo "</body></html>\n";
	}
   
	public function paypal_button() {
		$html = "<form method=\"post\" name=\"paypal_form\" ";
		$html .= "action=\"".$this->paypal_url."\">\n";
		if (isset($this->paypal_mail))
		{
			$html .= "<input type=\"hidden\" name=\"business\" value=\"".$this->paypal_mail."\"/>\n";
		}
		foreach ($this->fields as $name => $value) {
...
// 		}
	...
	public function log_ipn_results($success, $writeToLog = true) {
		...
		if ($writeToLog)
		{
			...
			$cartUid = isset($_POST["item_number"]) ? (string)$_POST["item_number"] : "";
